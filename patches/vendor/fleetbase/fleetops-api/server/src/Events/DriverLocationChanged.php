...
class DriverLocationChanged implements ShouldBroadcast
{
    ...
    /**
     * The uuid of the company the driver belongs to.
     *
     * Captured from the model so the company channel is correct even when the
     * broadcast is processed on a queue worker (where no HTTP session exists).
     *
     * @var string
     */
    public $companyUuid;
    ...
    public function __construct(Driver $driver, array $additionalData = [])
    {
        ...
        $this->companyUuid      = $driver->company_uuid;
    }
    ...
    {
        return [
            new Channel('company.' . ($this->companyUuid ?? session('company'))),
