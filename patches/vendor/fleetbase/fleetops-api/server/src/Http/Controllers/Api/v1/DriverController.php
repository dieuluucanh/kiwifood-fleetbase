...
class DriverController extends Controller
{
    ...
    public function track(string $id, Request $request)
    {
        ...
        // updateQuietly() above suppresses the model events that would normally
        // invalidate cached reads, so bust the live-map caches explicitly —
        // otherwise console poll fallbacks serve stale positions.
        try {
            LiveCacheService::invalidateMultiple(['drivers', 'vehicles']);
        } catch (\Throwable $e) {
            // never let cache invalidation break location tracking
        }
    ...
    public function toggleOnline(string $id, Request $request)
    {
        ...
        // updateQuietly() suppresses the model events that invalidate cached
        // reads — bust caches explicitly so the console reflects the new
        // online/tracking status immediately instead of minutes later.
        try {
            LiveCacheService::invalidateMultiple(['drivers', 'vehicles']);
            Driver::invalidateApiCacheManually($driver->company_uuid);
        } catch (\Throwable $e) {
            // never let cache invalidation break the toggle response
        }

        // Notify consoles in realtime that the driver's status changed
        // (location is unchanged; the delta carries the new online flag).
        broadcast(new DriverLocationChanged($driver, ['online' => $onlineValue]));
