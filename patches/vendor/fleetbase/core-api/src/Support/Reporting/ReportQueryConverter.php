    ...
    protected function buildSelectClause(Builder $query): void
    {
        ...
        if (!$hasGrouping) {
            ...
            foreach ($this->queryConfig['columns'] ?? [] as $column) {
                ...
                $isComputed  = !empty($column['computed']) && $column['computed'] === true;
                $computation = $column['computation'] ?? null;

                if ($isComputed && $computation) {
                    // Computed (aggregate) columns emit their resolved computation
                    // expression (e.g. SUM(distance_km)) rather than a phantom
                    // table.column reference.
                    $resolved     = $this->resolveComputedColumnReferences($computation, $rootTable);
                    $selects[]    = "({$resolved}) as `{$alias}`";
                    $hasAggregate = true;
                } else {
                    [$tblAlias, $col] = $this->resolveAliasAndColumn($rootTable, $name);
                    $selects[]        = "{$tblAlias}.{$col} as `{$alias}`";
                    $groupBys[]       = "{$tblAlias}.{$col}";
                }
            }

            // Add computed columns
            $this->buildComputedColumns($query, $selects);
            $hasAggregate = $hasAggregate || !empty($this->queryConfig['computed_columns']);

            if ($selects) {
                $query->selectRaw(implode(', ', $selects));
            }

            // Mixed plain + aggregate (computed) columns without an explicit
            // groupBy must be implicitly grouped by the non-aggregate columns
            // to satisfy ONLY_FULL_GROUP_BY; selecting only aggregates yields a
            // single rollup row (no group by).
            if ($hasAggregate && $groupBys) {
