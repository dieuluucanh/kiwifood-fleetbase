...
class Report extends Model
{
    ...
    protected function updateExecutionStats(float $executionTime, int $resultCount): void
    {
        ...
        // Older reports schemas (pre-migration) may not include the execution
        // statistics columns; drop them so the save still succeeds.
        if (!$this->reportsTableHasExecutionStatsColumns()) {
            $this->offsetUnset('execution_count');
            $this->offsetUnset('average_execution_time');
            $this->offsetUnset('last_result_count');
        }
    ...
    /**
     * Whether the reports table includes the execution statistics columns
     * (execution_count, average_execution_time, last_result_count). Cached per
     * process to avoid a schema query on every report execution.
     */
    protected function reportsTableHasExecutionStatsColumns(): bool
    {
        static $hasColumns = null;

        if ($hasColumns === null) {
            $hasColumns = \Illuminate\Support\Facades\Schema::hasColumn($this->getTable(), 'execution_count')
                && \Illuminate\Support\Facades\Schema::hasColumn($this->getTable(), 'average_execution_time')
                && \Illuminate\Support\Facades\Schema::hasColumn($this->getTable(), 'last_result_count');
        }
