...
| `fleetops-api/.../Controllers/Api/v1/DriverController.php` | **R3** — `track()` and `toggleOnline()` use `updateQuietly()`, which suppresses cache-invalidation events; console showed stale positions/online status for minutes. Now explicitly invalidates `LiveCacheService` (+ API model cache on toggle) and broadcasts the online change in realtime. |
