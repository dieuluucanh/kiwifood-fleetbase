...
| `core-api/.../routes.php` | **R7** — the console report-results page's **Export** buttons (XLSX/CSV/JSON) call `GET /reports/export-download/{filename}`, but the deployed image's `ReportController` has the `download()` method while `routes.php` is missing the route → every export download 404'd. Now adds the single line `$router->get('export-download/{filename}', $controller('download'))->name('reports.download')`. |
| `core-api/.../Support/Reporting/ReportQueryConverter.php` | **R8** — the console report **Export** buttons (Preview + Result views) POST to `/reports/export-query` and `/reports/{id}/export`, and the deployed image's `ReportController`/`Report` model call `ReportQueryConverter::getAvailableExportFormats()` and `::export()` — but both methods were only added in **core-api 1.6.55**, so on the pinned 1.6.54 every export threw an uncaught `Error: Call to undefined method` → HTML 500 → console "Error processing response: JSON.parse unexpected character". Now backports both methods (wrapping `execute()` + `ReportQueryExporter`) and aliases `xlsx` to `excel` so the console's "Excel (XLSX)" option passes format validation instead of 400ing. |
| `core-api/.../Models/Report.php` | **R8** — after the converter fix, the Result-view export (`Report::export()`) still failed because `logExport()` inserted a `details` column that doesn't exist on the deployed `report_audit_logs` schema (and its NOT NULL `user_uuid`/`company_uuid` columns aren't fillable) → `QueryException` → `success: false` 500. Now `logExport()` writes the payload to `metadata` and is guarded so audit logging can never fail an otherwise successful export. |
